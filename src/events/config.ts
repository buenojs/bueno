import { EventManagerConfig, EventManagerOptions, EventRegistryOptions } from './types';
...
export function validateEventSystemConfig(config: EventSystemConfig): boolean {
  ...
  if (config.manager.maxListeners !== undefined && 
      (typeof config.manager.maxListeners !== 'number' || config.manager.maxListeners < 0)) {
    return false;
  }

  // Validate registry config
  if (!config.registry || typeof config.registry !== 'object') return false;
  if (config.registry.maxEvents !== undefined && 
      (typeof config.registry.maxEvents !== 'number' || config.registry.maxEvents < 0)) {
    return false;
  }
  if (config.registry.retentionPeriod !== undefined && 
      (typeof config.registry.retentionPeriod !== 'number' || config.registry.retentionPeriod < 0)) {
    return false;
  }
  if (config.registry.cleanupInterval !== undefined && 
      (typeof config.registry.cleanupInterval !== 'number' || config.registry.cleanupInterval < 0)) {
    return false;
  }

  // Validate error handling
  if (config.errorHandling && !['throw', 'log', 'ignore'].includes(config.errorHandling)) {
    return false;
  }

  // Validate max event size
  if (config.maxEventSize !== undefined && 
...
export function loadEventSystemConfig(
  configPath: string = './config/event-system.json'
): EventSystemConfig {
  try {
